...
public class PolygonSphereTest {
  ...
  public void testRayIntersectFromInside() {
    ...
    assertThat(rayHit.get().getDistance()).isWithin(.01).of(1);
    Vector intersection = rayHit.get().getIntersection();
    assertThat(intersection.getX()).isWithin(0.01).of(1 / Math.sqrt(3));
    assertThat(intersection.getY()).isWithin(0.01).of(1 / Math.sqrt(3));
    assertThat(intersection.getZ()).isWithin(0.01).of(1 / Math.sqrt(3));
  }
  ...
  public void edgeTest() {
    PolygonSphere sphere = unitSphere.transform(translate(1.0, 0.0, -1.0));
