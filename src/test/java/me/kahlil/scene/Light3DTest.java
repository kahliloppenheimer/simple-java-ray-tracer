...
public class Light3DTest {
  ...
  private static final Sphere3D DUMMY_SPHERE =
      new Sphere3D(null, 0.0, null, null);

  /**
   * When computing the diffuse light at a point, it is possible that the dot product of the
   * surface normal and the light vector is negative. This means that the light should have no
   * effect at this angle, and thus the diffuse lighting should be zero.
   */
  @Test
  public void diffuseLightingIsZeroWhenSurfaceNormalDotLightIsNegative() {
    Light3D light = new Light3D(new Vector(0, 0, 0), Color.RED);
    RayHit rayHit = ImmutableRayHit.builder()
